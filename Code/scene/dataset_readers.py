...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, path, rgb_mapping, tele_dir):
    cam_infos = []
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # print("image_path", image_path, image_name, extr)
        # print("rgb_path", rgb_path, image_name)

        if 'train' in os.path.basename(rgb_path):
            if "Stereo" in rgb_path:
                tele_path = os.path.join(tele_dir, 
                                        os.path.basename(rgb_path).replace('train_sr', 'trainTele'))
            else:
                tele_path = os.path.join(tele_dir, 
                                        os.path.basename(rgb_path).replace('train', 'trainTele'))
            # print("tele_path", tele_path)

            tele_image = Image.open(tele_path)
        else:
            tele_image = None
        # print("rgb_path", rgb_path, image_name)
        
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, tele_image=tele_image, image_path=image_path,
...
def readColmapSceneInfo(path, images, eval, n_views=0, llffhold=10):
    ...
    tele_dir = os.path.join(path, 'tele')
    ...
    # print("cam_infos", len(cam_infos), [x.image_name for x in cam_infos])
    ...
    print("------------", "Number TrainView:", len(train_cam_infos), [x.image_name for x in train_cam_infos], "------------")
    print("------------", "Number TestView", len(test_cam_infos), [x.image_name for x in test_cam_infos],"------------")
