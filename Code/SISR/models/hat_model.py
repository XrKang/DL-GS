...
class HATModel(SRModel):
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img=True):
        dataset_name = dataloader.dataset.opt['name']
        save_path = dataloader.dataset.opt['save_path']
        try: pc_path = dataloader.dataset.opt['PC_path']
        except: pc_path = None
        with_metrics = self.opt['val'].get('metrics') is not None
        use_pbar = self.opt['val'].get('pbar', False)
        try: 
            tele_path = dataloader.dataset.opt['tele_path']
            tele_cp_path = dataloader.dataset.opt['tele_cp_path']
            print(tele_path, tele_cp_path)
            if not osp.exists(tele_cp_path):
                shutil.copytree(tele_path, tele_cp_path)
            else:
                shutil.rmtree(tele_cp_path)
                shutil.copytree(tele_path, tele_cp_path)
        except: pass
        try:
            dataroot_gt = dataloader.dataset.opt['dataroot_gt']
            gt_cp_path = dataloader.dataset.opt['gt_cp_path']
            print(dataroot_gt, gt_cp_path)
            if not osp.exists(gt_cp_path):
                shutil.copytree(dataroot_gt, gt_cp_path)
                # os.makedirs(gt_cp_path)
            else:
                shutil.rmtree(gt_cp_path)
                shutil.copytree(tele_path, tele_cp_path)
        except: pass
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                if self.opt['is_train']:
                    ...
                    print(osp.join(save_path, osp.splitext(osp.basename(val_data['lq_path'][0]))[0]+'_sr.jpg'))
                    save_img_path = osp.join(save_path, osp.splitext(osp.basename(val_data['lq_path'][0]))[0]+'_sr.jpg')
                    if 'train' in osp.splitext(osp.basename(val_data['lq_path'][0]))[0] and pc_path!=None:
                        save_img_path_2 = osp.join(pc_path, osp.splitext(osp.basename(val_data['lq_path'][0]))[0]+'_sr.jpg')
